...
class EE_APT_GET {
	...
	public static function update() {
		...
		$update = EE_CLI::exec_cmd( $update_cmd, 'apt-get update' );
		if ( 0 == $update ) {
			EE_CLI::success( 'Cache updated successfully.' );
		} else {
	...
	public static function check_upgrade() {
		...
		$check_upgrade = EE_CLI::exec_cmd( $check_upgrade_cmd, 'upgrade packages' );
		if ( 0 == $check_upgrade ) {
	...
	public static function dist_upgrade() {
		$dist_upgrade_cmd = 'DEBIAN_FRONTEND=noninteractive '.
                            ...
                            ' -y ';
		$dist_upgrade = EE_CLI::exec_cmd( $dist_upgrade_cmd, 'dist upgrade' );
		if ( 0 == $dist_upgrade ) {
			EE_CLI::success( 'Cache updated successfully.' );
		} else {
			EE_CLI::success( 'Oops Something went wrong!!' );
	...
	public static function install($packages) {
		...
		$install_package_cmd = 'sudo DEBIAN_FRONTEND=noninteractive ' .
			                     ...
			                     $all_packages;
		$install_package = EE_CLI::exec_cmd( $install_package_cmd, 'installing package' );
	...
	public static function remove($packages, $purge = false) {
		...
		$check_upgrade = EE_CLI::exec_cmd( $check_upgrade_cmd, 'removing packages' );
		if ( 0 == $check_upgrade ) {
			EE_CLI::success( 'Cache upgraded successfully.' );
		} else {
			EE_CLI::success( 'Oops Something went wrong!!' );
