...
class Containers {
	...
	private static function migrate_global_containers() {
		...
		if ( $existing_cron_image->return_code === 0 ) {
			self::$rsp->execute_step(
				...
				function () use ( $existing_cron_image ) {
	...
	private static function migrate_site_containers() {
		...
		foreach ( $sites as $site ) {
			...
			self::$rsp->execute_step(
				function () use ( $site, $docker_compose_path, $docker_compose_backup_path ) {
			...
			self::$rsp->execute_step(
				function () use ( $site, $docker_compose_backup_path, $docker_compose_path, $docker_compose_contents ) {
