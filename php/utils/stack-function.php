...
class EE_Stack {
	...
	public static function install( $assoc_args ) {
		...
		if ( ! empty( $stack['phpmyadmin'] ) ) {
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://github.com/phpmyadmin/phpmyadmin/archive/STABLE.tar.gz",
					"path"         => "/tmp/pma.tar.gz",
		...
		if ( ! empty( $stack['phpredisadmin'] ) ) {
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://github.com/ErikDubbelboer/phpRedisAdmin/archive/master.tar.gz",
					"path"         => "/tmp/pra.tar.gz",
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://github.com/nrk/predis/archive/v1.0.1.tar.gz",
		...
		if ( ! empty( $stack['adminer'] ) ) {
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://www.adminer.org/static/download/" . EE_ADMINER . "/adminer-" . EE_ADMINER . ".php",
		...
		if ( ! empty( $stack['utils'] ) ) {
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://storage.googleapis.com/google-code-archive-downloads/" .
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://raw.github.com/rlerdorf/" .
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://raw.github.com/amnuts/" .
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://gist.github.com/ck-on/4959032" .
			...
			$packages = array_merge( $packages, array(
				array(
					"url"          => "https://github.com/box/Anemometer/" .
					                  "archive/master.tar.gz",
...
//              todo: PHP and Debug pull configuration
		...
		if ( ! empty( $packages ) ) {
			foreach ( $packages as $package ) {
				...
				if ( in_array( '/tmp/anemometer.tar.gz', $packages ) ) {
					...
					$data = array(
						'host'     => EE_Variables::get_ee_mysql_host(),
						'port'     => '3306',
						'user'     => 'anemometer',
						'password' => $char
					);
	...
	public static function get_service_list( $assoc_args ) {
		$services = array();
		if ( ( array_key_exists('all',$assoc_args) && 1 == $assoc_args['all'] ) || ! empty( $assoc_args['nginx'] ) ) {
			if ( EE_Apt_Get::is_installed( 'nginx-custom' ) ) {
		...
		if ( ( array_key_exists('all',$assoc_args) && 1 == $assoc_args['all'] ) || ! empty( $assoc_args['redis'] ) ) {
			if ( EE_Apt_Get::is_installed( 'redis-server' ) ) {
		...
		if ( ( array_key_exists('all',$assoc_args) && 1 == $assoc_args['all'] ) || ! empty( $assoc_args['mysql'] ) )  {
		...
		if ( ( array_key_exists('all',$assoc_args) && 1 == $assoc_args['all'] ) || ! empty( $assoc_args['postfix'] ) ) {
			if ( EE_Apt_Get::is_installed( 'postfix' ) ) {
		...
		if ( ( array_key_exists('all',$assoc_args) && 1 == $assoc_args['all'] ) || ! empty( $assoc_args['memcached'] ) )  {
