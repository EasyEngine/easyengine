...
class Info_Command extends EE_Command {
	/**
	 * Invoke approriate command based on passed param.
	 *
	 * @since 4.0.0
	 *
	...
	public function info_php() {
		...
		if ( isset( $config['www']['php_admin_flag[xdebug.profiler_enable_trigger]'] ) ) {
	...
	public function info_mysql() {
		...
		$host = 'localhost';
		$port = EE::exec_cmd_output( "mysql -e \"show variables\" | grep ^port /etc/mysql/my.cnf | awk '{print($2)}' tr -d" );
